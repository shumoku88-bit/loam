...
/--
Transient latest-visible routing image for many subjects queried at one fixed
time. The retained RoutingHistory remains authority; this map exists only to
share the temporal selection scan across a bulk read.
-/
abbrev FixedTimeLatestIndex (Subject Time : Type) :=
  Std.HashMap String (RoutingEntry Subject Time)

private def fixedTimeChooseLatest
    (validOn : Time)
    (entry : RoutingEntry Subject Time)
    (current : Option (RoutingEntry Subject Time)) :
    Option (RoutingEntry Subject Time) :=
  if entry.effectiveOn ≤ validOn then
    match current with
    | none => some entry
    | some prior =>
        if prior.effectiveOn ≤ entry.effectiveOn then some entry else some prior
  else
    current

private def fixedTimeUpdate
    (subjectKey : Subject → String)
    (validOn : Time)
    (entry : RoutingEntry Subject Time)
    (index : FixedTimeLatestIndex Subject Time) :
    FixedTimeLatestIndex Subject Time :=
  if entry.effectiveOn ≤ validOn then
    match index.get? (subjectKey entry.subject) with
    | none => index.insert (subjectKey entry.subject) entry
    | some prior =>
        if prior.effectiveOn ≤ entry.effectiveOn then
          index.insert (subjectKey entry.subject) entry
        else
          index
  else
    index
...
/--
Build one transient latest-visible image for a fixed observation coordinate.
`subjectKey` is representation mechanics only; callers proving correspondence
must supply that it is injective.
-/
def buildFixedTimeLatestIndexBy
    (subjectKey : Subject → String)
    (history : RoutingHistory Subject Time)
    (validOn : Time) : FixedTimeLatestIndex Subject Time :=
  history.entries.foldr (fixedTimeUpdate subjectKey validOn) {}

private theorem fixedTimeUpdate_same
    (subjectKey : Subject → String)
    (validOn : Time)
    (entry : RoutingEntry Subject Time)
    (index : FixedTimeLatestIndex Subject Time) :
    (fixedTimeUpdate subjectKey validOn entry index).get?
        (subjectKey entry.subject) =
      fixedTimeChooseLatest validOn entry
        (index.get? (subjectKey entry.subject)) := by
  unfold fixedTimeUpdate fixedTimeChooseLatest
  by_cases hVisible : entry.effectiveOn ≤ validOn
  · simp only [if_pos hVisible]
    cases hPrior : index.get? (subjectKey entry.subject) with
    | none =>
        change
          (index.insert (subjectKey entry.subject) entry).get?
              (subjectKey entry.subject) = some entry
...
private theorem fixedTimeUpdate_other
    (subjectKey : Subject → String)
    (subjectKeyInjective : Function.Injective subjectKey)
    (validOn : Time)
    (entry : RoutingEntry Subject Time)
    (index : FixedTimeLatestIndex Subject Time)
    (subject : Subject)
    (hNe : entry.subject ≠ subject) :
    (fixedTimeUpdate subjectKey validOn entry index).get? (subjectKey subject) =
      index.get? (subjectKey subject) := by
  unfold fixedTimeUpdate
  have hKey : subjectKey entry.subject ≠ subjectKey subject := by
    intro h
    exact hNe (subjectKeyInjective h)
  by_cases hVisible : entry.effectiveOn ≤ validOn
  · simp only [if_pos hVisible]
    cases hPrior : index.get? (subjectKey entry.subject) with
    | none =>
        change
          (index.insert (subjectKey entry.subject) entry).get? (subjectKey subject) =
            index.get? (subjectKey subject)
        rw [Std.HashMap.get?_insert]
        have hBeq :
            (subjectKey entry.subject == subjectKey subject) = false :=
...
private theorem fixedTimeUpdate_lookup
    (subjectKey : Subject → String)
    (subjectKeyInjective : Function.Injective subjectKey)
    (validOn : Time)
    (entry : RoutingEntry Subject Time)
    ...
    (subject : Subject) :
    (fixedTimeUpdate subjectKey validOn entry index).get? (subjectKey subject) =
      combineLatest subject validOn entry (index.get? (subjectKey subject)) := by
  by_cases hSubject : entry.subject = subject
  · subst subject
    rw [fixedTimeUpdate_same]
    unfold combineLatest fixedTimeChooseLatest
    by_cases hVisible : entry.effectiveOn ≤ validOn
    · simp [hVisible]
    · simp [hVisible]
  · rw [fixedTimeUpdate_other subjectKey subjectKeyInjective validOn entry index subject hSubject]
    unfold combineLatest
    simp [hSubject]

private theorem buildFixedTimeLatestIndexBy_lookup_fold
    (subjectKey : Subject → String)
    (subjectKeyInjective : Function.Injective subjectKey)
    (entries : List (RoutingEntry Subject Time))
    (validOn : Time)
    (subject : Subject)
    (index : FixedTimeLatestIndex Subject Time) :
    (entries.foldr (fixedTimeUpdate subjectKey validOn) index).get?
        (subjectKey subject) =
      entries.foldr
        (combineLatest subject validOn)
        (index.get? (subjectKey subject)) := by
  induction entries generalizing index with
  | nil => rfl
  | cons entry rest ih =>
      simp only [List.foldr_cons]
      rw [fixedTimeUpdate_lookup subjectKey subjectKeyInjective]
      rw [ih]

/--
The fixed-time image lookup is exactly the canonical latest-visible selection.
This is the correctness bridge that permits a bulk reader to share one temporal
scan without changing routing semantics or authority.
-/
theorem buildFixedTimeLatestIndexBy_get?_eq_findLatestVisible?
    (subjectKey : Subject → String)
    (subjectKeyInjective : Function.Injective subjectKey)
    (history : RoutingHistory Subject Time)
    (subject : Subject)
    (validOn : Time) :
    (buildFixedTimeLatestIndexBy subjectKey history validOn).get?
        (subjectKey subject) =
      history.findLatestVisible? subject validOn := by
  unfold buildFixedTimeLatestIndexBy findLatestVisible?
  calc
    (history.entries.foldr (fixedTimeUpdate subjectKey validOn) {}).get?
          (subjectKey subject) =
        history.entries.foldr
          (combineLatest subject validOn)
          (({} : FixedTimeLatestIndex Subject Time).get? (subjectKey subject)) :=
      buildFixedTimeLatestIndexBy_lookup_fold
        subjectKey subjectKeyInjective history.entries validOn subject {}
    _ = history.entries.foldr (combineLatest subject validOn) none := by
      simp

/-- Project the ordinary three-way routing status from one fixed-time image. -/
def statusFromFixedTimeLatestIndexBy
    (subjectKey : Subject → String)
    (index : FixedTimeLatestIndex Subject Time)
    (subject : Subject) : RoutingStatus :=
  match index.get? (subjectKey subject) with
  | none => .unrouted
  | some entry =>
      match entry.purpose with
      | some purpose => .managed purpose
...
/--
Building then querying the transient image is pointwise identical to `statusAt`.
Focused callers should keep using `statusAt`; this theorem qualifies bulk
same-time readers only.
-/
theorem statusFromFixedTimeLatestIndexBy_build_eq_statusAt
    (subjectKey : Subject → String)
    (subjectKeyInjective : Function.Injective subjectKey)
    (history : RoutingHistory Subject Time)
    (subject : Subject)
    (validOn : Time) :
    statusFromFixedTimeLatestIndexBy subjectKey
        (buildFixedTimeLatestIndexBy subjectKey history validOn) subject =
      history.statusAt subject validOn := by
  unfold statusFromFixedTimeLatestIndexBy statusAt
  rw [buildFixedTimeLatestIndexBy_get?_eq_findLatestVisible?
    subjectKey subjectKeyInjective history subject validOn]
