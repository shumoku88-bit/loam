...
private def expectPlan
  ...
  match Loam.ScheduledCycleFill.plan
...
def run : IO Unit := do
  ...
  let shortLimit : Loam.ScheduledCycleFill.FillLimit := {
    endExclusive := "2026-03-15"
  }
  match Loam.ScheduledCycleFill.planCandidates shortLimit "2026-01-01"
  ...
  match Loam.ScheduledCycleFill.plan shortLimit "2026-01-01"
      { anchor := "2026-01-31", cadence := .monthly } with
  | .ok dates =>
      throw (IO.userError s!"day-31 monthly fill silently invented dates: {repr dates}")
  | .error _ => pure ()

  expect (Loam.ScheduledCycleFill.validResolvedDate
  ...
  let twoMonthLimit : Loam.ScheduledCycleFill.FillLimit := {
    endExclusive := "2026-04-15"
  }
  match Loam.ScheduledCycleFill.plan twoMonthLimit "2026-01-01"
