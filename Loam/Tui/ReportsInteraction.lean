...
/-!
# Production Reports interaction

This module owns report-menu state, query coordinates, and key-driven interaction
only. It consumes surface-independent Review answer types but performs no
household loading and renders no widgets.

Income & Expense composes the explicit RoleFlow boundary and Balances projects
the explicit RoleBalance boundary without adding named accounting engines.
Liquidity keeps its unconditional UNKNOWN baseline while exposing the read-only
conditional selected-balance path earned by Observations 229 and 231.
-/
