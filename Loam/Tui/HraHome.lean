...
private def helpLines (bounds : Bounds) : List Widget :=
  if bounds.width >= 120 then
    [mutedLine "[h/l] day  [k/j] week  [g] known  [Enter] day  [r] record  [a] actual  [p] scheduled  [i] attention  [c] budget  [e] capacity  [m] loci  [v] reports  [q] quit"]
  else
    [ mutedLine "[h/l] day  [k/j] week  [g] known  [Enter] day  [r] record  [q] quit"
    , mutedLine "[a] actual  [p] scheduled  [i] attention  [c] budget  [e] capacity  [m] loci  [v] reports"
    ]
