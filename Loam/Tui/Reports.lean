...
surface-independent shared review answers. Accounting and Liquidity remain visible
as explicit evidence-limit surfaces until production has legitimate source
evidence for those stronger questions.
-/
...
private def clearResults (state : State) : State :=
  { state with stockFlowSnapshot := none, budgetSnapshot := none }
...
def update (state : State) (key : Loam.Tui.Terminal.Key) : Step :=
  ...
  | .liquidity => updateEvidenceLimit state key
...
private def menuView (state : State) : Widget :=
  .column
    ...
    , menuRow state 2 "Liquidity" "future completeness currently unavailable"
...
private def liquidityView (state : State) : Widget :=
  .column
    ...
    , muted "What can LOAM safely say about the future liquid path?"
    , blank
    , line "Forecast path: UNKNOWN"
    , line "Known low-water mark: UNKNOWN"
    , blank
    , muted "Scheduled evidence exists, but production does not currently"
    , muted "justify a complete-future horizon for this report."
    , muted "Known Scheduled items are not silently treated as all future flows."
    , muted "No numeric path is extended from incomplete evidence."
    , blank
    , muted "b / Esc Reports menu   q quit"
    , line state.notice
    ]
