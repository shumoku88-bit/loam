...
partial def run
  ...
  if step.cancel then return "Scheduled creation cancelled."
  match step.publish with
  | some draft =>
      match ← Loam.ScheduledCreationPublisher.publishManifestCreation
          scheduledFile.toString root.toString draft with
      | .ok receipt =>
          return "Scheduled " ++ receipt.scheduled.token ++ " for " ++ receipt.scheduledOn ++ "."
      | .error message =>
          ...
          run bounds scheduledFile root known next nextFrame
  | none =>
      let nextFrame := compileWidget (Loam.Tui.ScheduledCreation.view known step.state)
      Loam.Tui.Terminal.emitDirtyDiff bounds 0 0 frame nextFrame
      run bounds scheduledFile root known step.state nextFrame
