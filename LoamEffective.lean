...
single-correction Core law.

No correction means the recorded aggregate is already the effective view.
Exactly one correction uses `EventCorrection.quantityAtEffective?`, which keeps
