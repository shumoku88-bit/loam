# Observation 165: Can settlement state collapse to an Event-to-relation discharge correspondence?

Status: bounded Alloy observation following independent Observations 163 and 164.

Observation 163 found recurring pressure to keep economic burden separate from physical payment and later shared-cost settlement. Observation 164 independently found a narrow common substrate under shared-cost receivables and deferred card payables: a directional open relation plus later discharge.

This observation asks whether that vocabulary can be reduced further before anything reaches the Practical Core.

## Question

Observation 164 used an observation-local `settled` set. A production design could easily turn that into new ontology:
...
If the expected matrix holds, Observations 163-165 support a smaller semantic picture than a transaction-kind hierarchy:
...
In particular, claim direction does not replace burden allocation. Observation 165 only asks whether settlement *state/object vocabulary* can be removed.
...
If current household questions can still be answered from existing movement evidence, keep Observation 165 as an observed boundary.
