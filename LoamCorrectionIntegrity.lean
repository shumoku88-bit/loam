...
Each correction relation is inspected independently. A relation is `closed`
only when both endpoint Events are present in the same EventMemory. For a closed
non-self relation, the existing effective-quantity law removes the original
contribution while retaining the replacement exactly once, because the
replacement is already a recorded Event.

This view deliberately does not choose a current winner among several
corrections and does not sum quantities whose locus/measure coordinates differ.
-/
