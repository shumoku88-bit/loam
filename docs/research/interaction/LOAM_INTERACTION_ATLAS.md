...
- `LOAM_INTERACTION_ATLAS_HOME_ATTENTION.md` — Home archetypes, attention, calendar/time navigation, quiet-state design
- `LOAM_INTERACTION_ATLAS_TRANSACTION_ENTRY.md` — cross-surface entry patterns and synthetic scenario metrics
- `LOAM_INTERACTION_ATLAS_HRA_ARCHAEOLOGY.md` — high-weight archaeology of the user's easiest prior household TUI
- `LOAM_INTERACTION_ATLAS_HKERNEL_ARCHAEOLOGY.md` — comparative h-kernel TUI archaeology and HRA/h-kernel interaction-depth analysis
- `LOAM_INTERACTION_ATLAS_SUSTAINED_USE.md` — interruption/resumption, prospective memory, notification fatigue, automation trust, undo/history, direct manipulation, feedback, and long-term use
...
|---|---|---|
| HRA | OBSERVED first pass | compare measured traces against h-kernel and LOAM |
| h-kernel | OBSERVED first pass | measure focus depth and scenario traces against HRA |
...
1. Run HRA vs h-kernel vs current LOAM interaction traces over the same synthetic scenarios.
2. Deepen the **Difficult Operations Catalog** where those traces expose friction.
3. Use the **Home / Attention Atlas** to compare HRA's flatter date-and-object topology against h-kernel's persistent focus shell.
4. Use the **Transaction Entry Atlas** to compare one general editor against specialized intent-first flows.
