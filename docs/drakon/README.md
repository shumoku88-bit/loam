...
Open that file in DRAKON Editor. The primary `loam-system-map.drn` is a local generated artifact and is ignored by Git; `build_map.py` is its deterministic, reviewable source. The read-path map and focused TUI CLI / legacy-workspace audit maps follow the same builder-only rule. A small set of module-granularity `.drn` files remains intentionally tracked as golden artifacts where CI compares regenerated bytes with the committed copy.
