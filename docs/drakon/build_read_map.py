...
READ_FLOW_DIAGRAMS = {
    ...
    "07.1.1 Actual Review Read Boundary": {
        ...
        "nodes": [
            ...
            ("action", "Reuse carried currentValidities\nno second validity admission"),
            ("action", "Keep ALL retained historical Events\nnot only currentEvents"),
            ("action", "For each remembered Event\nproject transient review Record"),
            ("action", "date := carried current validity\nreplacement := admitted target lookup"),
            ("action", "derive isCurrent := replacement.isNone\nG2-024 preserved"),
            ("action", "description := retained Actual description evidence"),
    ...
    "07.3.1 Role Balance Read Boundary": {
        ...
        "nodes": [
            ...
            ("decision", "Full normalized Actual admission succeeded?", "Refuse\nmalformed or unsupported Actual evidence"),
            ("insertion", "Load ZeroOriginCoverage + OpeningSupport\n+ CurrentQuantityAnchor + AccountingRole"),
            ("decision", "Independent support evidence decodes?", "Refuse\nmalformed or unsupported support evidence"),
            ("action", "Ordinary current world := image.currentEvents\nno RoleBalance correction re-admission"),
            ("decision", "Opening witnesses + support-family separation valid?", "Refuse\nunsupported or overlapping evidence"),
            ("action", "Route candidate coordinates ONCE\nzero / opening / anchor / unsupported"),
            ("insertion", "Zero-origin bucket\nBalanceReview.projectImage(image)"),
            ("action", "Opening bucket\nquantityAtRecorded(image.currentEvents)"),
            ("insertion", "Anchor bucket\nretained Events + Corrections -> reflected-root delta frontier"),
            ("action", "Compose supported quantities + AccountingRole\nkeep unsupported/unresolved frontiers visible"),
            ("action", "Return one RoleBalance Snapshot"),
        ],
    },
