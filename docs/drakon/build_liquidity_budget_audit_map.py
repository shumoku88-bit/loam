...
DIAGRAMS = {
    "G2.007.1 Liquidity / Budget Window Comparison": {
        ...
        "audit": "Conditional Liquidity composes two Actual-backed readers and therefore needs one Actual generation across both reads. Budget Window loads one Actual evidence image but fans a query-global correction world into Purpose-local arithmetic. Similar report surfaces expose different obligation pressure.",
        "nodes": [
    ...
    "G2.007.3 Budget Window Shared Frontier": {
        "description": "Separate query-global correction admission from Purpose-local window arithmetic.",
        "sources": "Loam/BudgetWindowReview.lean; Loam/Application/CapacityWindowInspection.lean; Loam/Application/ConsumptionInspection.lean; Loam/Application/CorrectionFrontier.lean",
        "audit": "The correction frontier depends only on the shared Event/correction evidence, not Purpose. An empty Purpose set must not force it. For a non-empty set, the first Purpose keeps Entitlement-before-Consumption refusal order, then one admitted frontier is shared across all Purpose-local Consumption folds. Purpose arithmetic remains local and is not replaced by a new aggregation engine.",
        "nodes": [
            ...
            ("decision", "first Entitlement admitted?", "refuse projection"),
            ("insertion", "correctionFrontierMemory? ONCE"),
            ("decision", "first Consumption admitted?", "refuse projection"),
            ("action", "first Row"),
            ("action", "remaining Purposes left-to-right"),
            ("decision", "Purpose Entitlement admitted?", "refuse projection"),
            ("action", "Consumption from SAME frontier"),
